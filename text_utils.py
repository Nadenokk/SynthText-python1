...
def move_bb(bbs, t):
    ...
    Переведите ограничивающие блоки в t_x, t_y.
     BB: 2x4xn
     T: 2-длинный np.array
    """
...
def crop_safe(arr, rect, bbs=[], pad=0):
    ...
    ARR: обр
     RECT: (x, y, w, h): область для обрезки
     BBS: ограничительные рамки формата nx4 xywh
     PAD: процент к пэду

     Делает безопасную обрезку. Возвращает обрезанный прямоугольник и
     настраиваемые ограничительные рамки
    """
    rect = np.array(rect)
    rect[:2] -= pad
    rect[2:] += 2*pad
...
class BaselineState(object):
    ...
    def get_sample(self):
        ...
        Возвращает функции для кривой и дифференциала для a и b
        """
        sgn = 1.0
...
class RenderFont(object):
    ...
    Outputs a rasterized font sample.
        ...
        Выводит растеризованный образец шрифта.
        Вывод - это двоичная матрица маски, обрезанная близко к шрифту.
        Также выводит ограничивающие прямоугольники и текстовую строку
    """

    def __init__(self, data_dir='data'):
        # distribution over the type of text:
        # whether to get a single word, paragraph or a line:
        # Распределение по типу текста:
        # получить ли одно слово, абзац или строку:
        ...
        # минимальное количество символов, которое должно поместиться в маске
        # чтобы определить максимальную высоту шрифта.
        self.min_nchar = 2
        self.min_font_h = 16 #px : 0.6*12 ~ 7px <= actual minimum height
        # px: 0.6 * 12 ~ 7px <= фактическая минимальная высота
        self.max_font_h = 120 #px
        self.p_flat = 0.10

        # curved baseline:
        # изогнутая базовая линия:
        self.p_curved = 1.0
        self.baselinestate = BaselineState()

        # text-source : gets english text:
        # text-source: получает текст на английском языке:
        self.text_source = TextSource(min_nchar=self.min_nchar,
                                      fn=osp.join(data_dir,'newsgroup/newsgroup.txt'))

        # get font-state object:
        # получить объект состояния шрифта:
    ...
    def render_multiline(self,font,text):
        ...
        отображает многострочный текст на поверхности пигмея SURF с
         Стиль шрифта FONT.
         Новая строка в тексте обозначается \ n, остальные символы
         убежали. Другие формы пробелов должны быть преобразованы в пространство.

         возвращает обновленную поверхность, слова и ограничивающие рамки для символов.
        """
        ...
        # инициализировать поверхность до нужного размера:
        ...
        # получить объединение символов для обрезки:
        ...
        # обрезать поверхность по размеру текста:
        bbs = np.array(bbs)
    ...
    def render_curved(self, font, word_text):
        ...
        использовать кривую базовую линию для рендеринга слова
        """
        wl = len(word_text)
        isword = len(word_text.split())==1

        # do curved iff, the length of the word <= 10
        # сделать изогнутый, если длина слова <= 10
        ...
        # рендерить символы влево и вправо:
        last_rect = rect
        ch_idx = []
        for i in range(wl):
            #skip the middle character
            # пропустить средний символ
            if i==mid_idx: 
        ...
        # исправить порядок ограничивающего прямоугольника:
    ...
    def get_nline_nchar(self,mask_size,font_height,font_width):
        ...
        Возвращает максимальное количество строк и символов, которые могут поместиться
         на изображении MASK_SIZED.
        """
        H,W = mask_size
    ...
    def place_text(self, text_arrs, back_arr, bbs):
        ...
        for i in order:            
            ...
            if not np.any(safemask): # no collision-free position:
                # нет положения без столкновений:
                #warn("COLLISION!!!")
            ...
            # обновить ограничивающие рамки:
            bbs[i] = move_bb(bbs[i],loc[::-1])

            # blit the text onto the canvas
            # перетащить текст на холст
    ...
    def bb_xywh2coords(self,bbs):
        ...
        Принимает матрицу ограничивающего прямоугольника nx4, указанную в x, y, w, h
        форматирует и выводит 2x4xn bb-матрицу (4 вершины на bb).
        """
        n,_ = bbs.shape
    ...
    def render_sample(self,font,mask):
        ...
        Размещает текст в «свободной от столкновений» области, как указано
         в маске - 255 для небезопасных, 0 для безопасных.
         Текст визуализируется с использованием шрифта, текстовое содержимое - TEXT.
        """
        #H,W = mask.shape
        ...
        # найти максимальную высоту в пикселях:
        ...
        if max_font_h < self.min_font_h: # not possible to place any text here
            # здесь невозможно разместить текст
            return #None
        # Никто

        # let's just place one text-instance for now
        ## TODO : change this to allow multiple text instances?
        # давайте сейчас просто разместим один экземпляр текста
        ## TODO: изменить это, чтобы разрешить несколько текстовых экземпляров?
        i = 0
        while i < self.max_shrink_trials and max_font_h > self.min_font_h:
            ...
            # выборка произвольной высоты шрифта:
            ...
            # преобразовать высоту пикселя в размер шрифта:
            ...
            # вычислить максимальное количество строк / символов на строку:
...
class FontState(object):
    ...
    Defines the random state of the font rendering
     ...
     Определяет случайное состояние рендеринга шрифта
    """
    size = [50, 10]  # normal dist mean, std
    # нормальное дистанционное среднее, стандартное
    underline = 0.05
    strong = 0.5
    oblique = 0.2
    wide = 0.5
    strength = [0.05, 0.1]  # uniform dist in this interval
    # равномерное расстояние в этом интервале
    ...
    # бета-распределение альфа, бета, смещение, диапазон (среднее значение a / (a + b))
    border = 0.25
    random_caps = -1 ## don't recapitalize : retain the capitalization of the lexicon
    ## не рекапитализировать: сохранить капитализацию лексикона
    capsmode = [str.lower, str.upper, str.capitalize]  # lower case, upper case, proper noun
    # нижний регистр, верхний регистр, собственное существительное
    curved = 0.2
    random_kerning = 0.2
    ...
    def __init__(self, data_dir='data'):
        ...
        # получить символьные частоты на английском языке:
        ...
        # получить модель для преобразования из пикселя в размер шрифта pt:
        ...
        # получить имена шрифтов для использования:
    ...
    def get_aspect_ratio(self, font, size=None):
        ...
        Возвращает медианное соотношение сторон каждого символа шрифта.
        """
        if size is None:
            size = 12 # doesn't matter as we take the RATIO
            # не имеет значения, как мы принимаем соотношение
        chars = ''.join(self.char_freq.keys())
        w = np.array(self.char_freq.values())

        # get the [height,width] of each character:
        # получить [высоту, ширину] каждого символа:
        try:
    ...
    def get_font_size(self, font, font_size_px):
        ...
        Возвращает размер шрифта, который соответствует высоте шрифта FONT_SIZE_PX пикселей.
        """
    ...
    def sample(self):
        ...
        Образцы из распределения состояния шрифта
        """
        return {
    ...
    def init_font(self,fs):
        ...
        Инициализирует шрифт Pygame.
        FS: образец состояния шрифта
        """
...
class TextSource(object):
    ...
    Предоставляет текст для слов, абзацев, предложений.
    """
    def __init__(self, min_nchar, fn):
        """
        TXT_FN : path to file containing text data.

        TXT_FN: путь к файлу, содержащему текстовые данные.
        """
        ...
        # распределение по строке / словам для LINE / PARA:
        ...
        # вероятность выравнивания по центру абзаца:
    ...
    def check_symb_frac(self, txt, f=0.35):
        ...
        Возврат T / F: доля Tiff символа / специальных символов в
        txt меньше или равна f (по умолчанию = 0,25).
        """
    ...
    def is_good(self, txt, f=0.35):
        ...
        T / F return: если строки в txt (список строк txt)
                      являются «действительными».
                      Данная строка l действительна, если:
                          1. Это не пусто.
                          2. символ_фракция> f
                          3. Имеет как минимум символы self.min_nchar
                          4. Не все символы i, x, 0, O, -
        """
        def is_txt(l):
            char_ex = ['i','I','o','O','0','-']
    ...
    def center_align(self, lines):
        ...
        Линии PADS с пробелом по центру
         линии: список текстовых строк.
        """
    ...
    def sample_para(self,nline_max,nchar_max):
        ...
        if lines is not None:
            ...
            # центр выравнивания текста абзаца:
