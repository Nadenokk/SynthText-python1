...
Скрипт для быстрой реконструкции изображения по градиентам.
На основе сценария Матлаба Рамеша Раскара, доступного здесь:
http://web.media.mit.edu/~raskar/photo/code.pdf

Немного адаптирован для «смешанного» редактирования Пуассона. [Perez et al.]
Документ: http://www.cs.jhu.edu/~misha/Fall07/Papers/Perez03.pdf
"""
...
def DST(x):
    ...
    Преобразует выход DST Сципи в DST (масштабирование) Matlab.
    """
...
def poisson_solve(gx,gy,bnd):
    ...
    # установить внутреннюю границу изображения на 0:
    bnd[1:-1,1:-1] = 0
    # get the boundary laplacian:
    # получить границу лапласиана:
    ...
    # сначала вдоль столбцов, затем вдоль строк
    ...
    # обратное DST для строк и столбцов
...
def blit_images(im_top,im_back,scale_grad=1.0,mode='max'):
    ...
    объединить изображения с помощью редактирования Пуассона.
    IM_TOP и IM_BACK должны быть одинакового размера.
    """
    ...
    # градиент градиентов из источника:
    for ch in range(im_top.shape[2]):
        ...
        # смешать исходный и целевой градиенты:
        if mode=='max':
            gx = gxs.copy()
            ...
            # получить статистику градиентной смеси:
...
def contiguous_regions(mask):
    ...
    вернуть список (ind0, ind1) такой, что маска [ind0: ind1] .all ()
    Правда и мы покрываем все такие регионы
    """
    in_region = None
    boundaries = []
