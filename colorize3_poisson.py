...
class Layer(object):
    ...
    def __init__(self,alpha,color):
        ...
        if color.ndim==1: # constant color for whole layer
            # постоянный цвет для всего слоя
            ncol = color.size
...
class FontColor(object):
    ...
    def __init__(self, col_file):
        ...
        # computations:
        # конвертировать color-означает из RGB в LAB для лучшего ближайшего соседа
        # вычисления:
    ...
    def sample_normal(self, col_mean, col_std):
        ...
        выборка из нормального распределения, сосредоточенного вокруг COL_MEAN
        со стандартным отклонением = COL_STD.
        """
    ...
    def sample_from_data(self, bg_mat):
        ...
        bg_mat: это изображение RGB размером nxmx3.
        
        возвращает кортеж: (RGB_foreground, RGB_background)
        каждый из них является 3-вектором.
        """
        bg_orig = bg_mat.copy()
        ...
        # выберите случайный цвет среди трех самых близких совпадений:
        # nn = np.random.choice (np.argsort (норм) [: 3])
        nn = np.argmin(norms)

        ## nearest neighbour color:
        ## цвет ближайшего соседа:
        ...
        if nn < self.ncol:
            ...
            # нужно поменять местами, чтобы второй цвет приблизился к входному цвету фона
            return (col1, col2)
    ...
    def complement(self, rgb_color):
        ...
        вернуть цвет, который дополняет RGB_COLOR.
        """
    ...
    def triangle_color(self, col1, col2):
        ...
        Возвращает цвет, который "противоположен" как col1, так и col2.
        """
...
class Colorize(object):
    ...
    def drop_shadow(self, alpha, theta, shift, size, op=0.80):
        ...
        альфа: альфа-слой, тень которого нужно отбрасывать
         тета: [0,2pi] - направление тени
         shift: сдвиг в пикселях тени
         size: размер фильтра GaussianBlur
         op: непрозрачность тени (множитель)

         @return: альфа теневого слоя
                   (предполагается, что цвет черный / белый)
        """
        if size%2==0:
            size -= 1
            size = max(1,size)
    ...
    def border(self, alpha, size, kernel_type='RECT'):
        ...
        альфа: альфа-слой текста
        размер: размер ядра
        тип ядра: один из [прямоугольник, эллипс, крест]

        @return: альфа-слой границы (цвет, добавляемый извне).
        """
    ...
    def merge_two(self,fore,back,blend_type=None):
        ...
        объединить два слоя FOREground и BACKground.
        ссылка: https://en.wikipedia.org/wiki/Alpha_compositing
        ссылка: глава 7 (стр. 440 и стр. 444):
             http://partners.adobe.com/public/developer/en/pdf/PDFReference.pdf
        """
    ...
    def merge_down(self, layers, blends=None):
        ...
        слои: [l1, l2, ... ln]: список объектов LAYER.
                  l1 находится сверху, ln - самый нижний слой.
         смесь: тип смеси для использования. Должно быть n-1.
                  используйте None для простого альфа-смешивания.
         Примечание: (1) предполагается, что все слои имеют одинаковый размер.
         @return: один объект типа LAYER, представляющий объединенное изображение
        """
        nlayers = len(layers)
        if nlayers > 1:
...
             - может быть таким же, как цвет текста, но ниже / выше компонента VALUE.
             - может быть таким же, как bg-color, но ниже / выше 'VALUE'.
             - может быть «средний» цвет ч / б текст и bg цвета.
        """
        ...
        # сначала выберите цвет, затем inc / dec его ЗНАЧЕНИЕ:
        if choice==0:
            # increase/decrease saturation:
            # увеличить / уменьшить насыщенность:
...
             - может быть какое-то другое случайное изображение.
             - может быть цвет в зависимости от фона.
                 этот цвет взят из словаря
                 из цветов текстовых изображений. ЦЕННЫЙ КАНАЛ
                 рандомизирован.

             H: минимальная высота символа
        """
...
        вернуть text_arr blit на bg_arr.
    ...
    def check_perceptible(self, txt_mask, bg, txt_bg):
        ...
        --- УСТАРЕЛО; ИСПОЛЬЗОВАТЬ ПРОВЕРКУ ГРАДИЕНТА В POISSON-RECONSTRUCT INSTEAD ---

         проверяет, является ли текст после слияния с фоном
         все еще видно
         txt_mask (hxw): двоичное изображение текста - 255, где присутствует текст
                                                    0 в другом месте
         bg (hxwx3): оригинальное фоновое изображение без текста.
         txt_bg (hxwx3): изображение с текстом.
        """
    ...
    def color(self, bg_arr, text_arr, hs, place_order=None, pad=20):
        ...
        Вернуть раскрашенное текстовое изображение.

         text_arr: список (n x m) альфа-маски с пустым текстом (unit8).
         hs: список минимальных высот (скалярных) символов в каждом текстовом массиве.
         text_loc: [строка, столбец]: расположение текста на холсте.
         canvas_sz: размер изображения холста.
        
         return: nxmx3 rgb colorized text-image.
        """
        bg_arr = bg_arr.copy()
        ...
        # инициализировать порядок размещения:
        ...
        for i in place_order[::-1]:
            ...
            # получить "местоположение" текста на изображении:
            ## это минимальные координаты x и y текста:
