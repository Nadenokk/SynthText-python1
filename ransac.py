...
def fit_plane(xyz,z_pos=None):
    ...
    если z_pos не None, знак
    нормаль переворачивается, чтобы сделать
    скалярное произведение с z_pos (+).
    """
    ...
    # единица-норма самолет-норма:
    abcd =  np.r_[abc,d]/np.linalg.norm(abc)
    # flip the normal direction:
    # переверните нормальное направление:
...
def fit_plane_ransac(pts, neighbors=None,z_pos=None, dist_inlier=0.05, 
    ...
    Подходит для трехмерной модели самолета, используя RANSAC.
    pts: (массив nx3) координат точек
    """
    n,_ = pts.shape
    ninlier,models = [],[]
    ...
    if models == []:
        ...
        for idx in best_model_idx[:min(10,len(best_model_idx))]:
            ...
            # переоценить модель на основе вкладышей:
